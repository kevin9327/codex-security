...
`openWindowsReadFile` supplies chunked binary reads for Deep Scan snapshot comparison. Its owned CRT descriptor preserves [Python 3.12 read errors](https://github.com/python/cpython/blob/v3.12.10/Python/fileutils.c), file sharing and close behavior. The Windows proof checks raw filenames, chunk boundaries, EOF, lock contention and closed descriptors.
...
`clearFileFlags(path, followSymlinks)` clears macOS file flags through `chflags` or `lchflags`, returning the native errno. This lets the temporary-checkout owner reset copied immutable flags before chmod and deletion, following [Python 3.12 temporary-directory cleanup](https://github.com/python/cpython/blob/v3.12.10/Lib/tempfile.py). Other Unix hosts return `ENOTSUP`; cleanup policy remains with the caller.

`chmodNoFollow(path, mode)` calls macOS `lchmod` and returns the native errno. Unlike Node's implementation, it does not open the path for writing, so cleanup can restore permissions on read-only files and directories without following symlinks. Other Unix hosts return `ENOTSUP`.
...
`createWindowsHardLink`, `replaceWindowsPath`, and `unlinkWindowsPath` preserve the path-based [CPython 3.12 filesystem calls](https://github.com/python/cpython/blob/v3.12.10/Modules/posixmodule.c) used to publish Deep Scan artifacts. They return the original Win32 errors and retain UTF-16LE paths. Unlink removes a file, directory symlink, or junction while retaining the target of a link. The Windows proof covers raw filenames, hard-link identities, overwrite and sharing behavior, read-only files, directory links, and error codes.

`createWindowsPrivateDirectory` preserves [CPython 3.12's `mkdir(mode=0700)`](https://github.com/python/cpython/blob/v3.12.10/Modules/posixmodule.c): a protected inheritable DACL grants full control to SYSTEM, Administrators, and the owner. It returns the CreateDirectory error with the original UTF-16LE path; descriptor conversion or release errors omit the path. The scan temporary-directory helper uses this operation. The Windows proof reads back the installed DACL through its test-only launcher.

`windowsInvariantLowercase` uses `LCMapStringEx` with the invariant locale and filesystem lowercase mapping, matching Python's `ntpath.normcase`. The typed scan-local file backend uses it when comparing verified handle paths. That backend retains ancestor handles without delete sharing and renames or deletes the opened leaf handle during finalization and artifact publication.
...
`process-binding.mts` exposes synchronous `rawProcess` for raw POSIX bytes or Windows UTF-16LE executable, arguments, cwd, and inherited environment edits. It captures unlimited byte stdout/stderr concurrently with byte stdin; absent input inherits stdin and an empty buffer sends EOF. Git diffs stream into an owned `openProcessOutput` spool, unlinked immediately on Unix and deleted on final handle closure on Windows, including helper termination. Spawn failures return native error numbers; successful calls return Python-compatible exit or signal codes. Unix restores SIGPIPE/SIGXFSZ and closes inherited descriptors at exec. Windows restricts handle inheritance and preserves CreateProcess executable lookup. It does not request a shell. Typed Git, patch and workbench helpers use this primitive.
...
`sqlite.mts` accepts the native binding and provides the connection, statement, row, transaction, scalar-function, and online-backup primitives used by the workbench and snapshot helper. SQLite INTEGER values remain bigint and REAL values remain number, including integral REAL values; callers choose any domain conversion explicitly. Transactions require synchronous callbacks. Queries, migrations, and command routing remain in TypeScript.
