...
When the host selects a source MCP, use it for source reads, searches, and inherited `SECURITY.md` guidance at the exact approved repository and base/head revisions. Pass that source-access context to every phase and worker. Use the host-prepared committed inventory, including deletions; keep shell commands offline and report inaccessible revisions or incomplete reads as evidence gaps. This replaces local source inspection in the phase references.
