...
- **TypeScript helpers:** unit and CLI tests cover capability preflight, scan contract finalization, rank-input generation and selection, and report format validation.
- **Schema validation:** Draft 2020-12 validators and checked-in completed-scan examples are validated with Ajv, including RFC 3339 formats.
...
- **CLI and harness dry runs:** helper commands retain `--help` coverage; major command paths run against temporary repositories and scan bundles without a model call.
- **Static checks:** TypeScript compilation, formatting, and portable source checks cover the plugin helpers and tests.
