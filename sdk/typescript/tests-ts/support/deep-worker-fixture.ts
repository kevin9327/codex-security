...
import * as workerOwner from "../../../../plugins/codex-security/mcp-app/src/workbench-deep-worker";
import * as dedupOwner from "../../../../plugins/codex-security/mcp-app/src/workbench-deep-dedup-claim";
...
export interface Action {
  operation: "upsert" | "dedup" | "sql" | "query" | "commit" | "rollback";
  args?: Partial<
    workerOwner.UpsertDeepWorkerArguments & dedupOwner.ClaimDeepDedupArguments
  >;
  timestamp?: string;
  failNow?: number;
  nowSql?: string;
...
function execute(request: Request): Response {
  ...
  try {
    ...
    const outcomes = request.actions.map((action): Outcome => {
      ...
      const context: workerOwner.DeepWorkerContext = {
        now: () => {
      ...
      const args: workerOwner.UpsertDeepWorkerArguments &
        dedupOwner.ClaimDeepDedupArguments = {
        scanId,
        workerId,
        coordinatorGeneration: null,
        kind: "discovery",
        status: "queued",
        promptPath: appendPath(directory, "prompt.txt"),
        artifactDir: appendPath(directory, "worker"),
      ...
      try {
        ...
        switch (action.operation) {
          case "upsert":
            result = workerOwner.upsertDeepScanWorker(
              context,
              connection,
              args,
            );
            break;
          case "dedup":
            result = dedupOwner.claimDeepScanDedup(context, connection, args);
            break;
          case "sql":
