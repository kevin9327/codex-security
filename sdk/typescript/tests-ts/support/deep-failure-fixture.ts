...
import * as owner from "../../../../plugins/codex-security/mcp-app/src/workbench-deep-failure";
...
export interface Action {
  operation:
    ...
    | "commit"
    | "rollback";
  args?: Partial<owner.FailDeepScanArguments>;
  timestamp?: string;
  failNow?: number;
  nowSql?: string;
...
function execute(request: Request): Response {
  ...
  try {
    ...
    const outcomes = request.actions.map((action): Outcome => {
      ...
      const context: owner.DeepFailureContext = {
      ...
      const args: owner.FailDeepScanArguments = {
        scanId,
      ...
      try {
        ...
        switch (action.operation) {
          case "failure":
            result = owner.failDeepScan(context, connection, args);
            break;
          case "locked":
            result = owner.failDeepScanLocked(
              context,
              connection,
              args,
              args.scanId,
            ...
            break;
          case "publication":
            result = owner.recordDeepScanPublicationFailure(
              context,
              connection,
              args,
            );
