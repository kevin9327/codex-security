...
import * as stop from "../../../../plugins/codex-security/mcp-app/src/workbench-scan-stop";
    ...
    const outcomes = request.actions.map((action): Outcome => {
      ...
      try {
        ...
        switch (action.operation) {
          ...
          case "failLocked":
            result = (
              action.operation === "fail" ? stop.failScan : stop.failScanLocked
            )(db, connection, {
              scanId: id,
          ...
          case "cancelLocked":
            result = (
              action.operation === "cancel"
                ? stop.cancelScan
                : stop.cancelScanLocked
            )(db, connection, {
              scanId: id,
              threadId: action.threadId ?? null,
            });
            break;
          case "preserveStopped":
            stop.preserveStoppedResultsAfterTransition(db, connection, id);
            break;
          case "sql":
