...
import * as kickoff from "../../../../plugins/codex-security/mcp-app/src/workbench-scan-kickoff";
import { scanTargetRoot } from "../../../../plugins/codex-security/mcp-app/src/workbench-setup";
...
export interface Action {
  operation:
    ...
    | "commit"
    | "rollback";
  args?: Partial<kickoff.StartScanArguments & kickoff.PromptScanArguments>;
  now?: string;
  nowSql?: string;
  failNow?: number;
  uuids?: string[];
    ...
    const outcomes = request.actions.map((action): Outcome => {
      ...
      try {
        ...
        switch (action.operation) {
          case "start":
            result = kickoff.startScan(context, connection, args);
            break;
          case "prompt":
            result = kickoff.startPromptOnlyScan(context, connection, args);
            break;
          case "headless":
            result = kickoff.startHeadlessStandardScan(
              context,
              connection,
              args,
            );
            break;
          case "root":
            result = scanTargetRoot(args.scanRoot, args.targetPath);
            break;
          case "sql":
