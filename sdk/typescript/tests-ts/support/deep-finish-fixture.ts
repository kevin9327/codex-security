...
import * as owner from "../../../../plugins/codex-security/mcp-app/src/workbench-deep-finish";
...
export interface Action {
  operation: "finish" | "locked" | "sql" | "query" | "commit" | "rollback";
  args?: Partial<owner.FinishDeepScanArguments>;
  timestamp?: string;
  failNow?: number;
  nowSql?: string;
...
function execute(request: Request): Response {
  ...
  try {
    ...
    const outcomes = request.actions.map((action): Outcome => {
      ...
      const context: owner.DeepFinishContext = {
      ...
      const args: owner.FinishDeepScanArguments = {
        scanId,
        coordinatorGeneration: null,
        manifestPath: appendPath(directory, "finish.json"),
      ...
      try {
        ...
        switch (action.operation) {
          case "finish":
            result = owner.finishDeepScan(context, connection, args);
            break;
          case "locked":
            result = owner.finishDeepScanLocked(
              context,
              connection,
              args,
              args.scanId,
