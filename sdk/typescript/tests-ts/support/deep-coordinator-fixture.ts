...
import * as coordinator from "../../../../plugins/codex-security/mcp-app/src/workbench-deep-coordinator";
...
export interface Action {
  operation:
    ...
    | "commit"
    | "rollback";
  args?: Partial<coordinator.ClaimCoordinatorArguments>;
  timestamp?: string;
  failNow?: number;
  nowSql?: string;
...
function execute(request: Request): Response {
  ...
  try {
    ...
    const outcomes = request.actions.map((action): Outcome => {
      ...
      const context: coordinator.DeepCoordinatorContext = {
        now: () => {
      ...
      const args: coordinator.ClaimCoordinatorArguments = {
        scanId,
        threadId: "owner",
        claimToken: null,
      ...
      try {
        ...
        switch (action.operation) {
          case "claim":
            result = coordinator.claimDeepScanCoordinator(
              context,
              connection,
              args,
            );
            break;
          case "locked":
            result = coordinator.claimDeepScanCoordinatorLocked(
              context,
              connection,
              args,
              args.scanId,
            ...
            break;
          case "lease":
            result = coordinator.coordinatorLeaseIsLive(
              connection,
              run(),
              connection
            ...
            break;
          case "recover":
            coordinator.recoverExpiredCoordinator(
              connection,
              run(),
              action.timestamp ?? now,
            );
            break;
          case "ledger":
            coordinator.recoverCandidateLedgerPublication(
              connection,
              args.scanId,
            );
            break;
