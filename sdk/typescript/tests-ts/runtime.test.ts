...
describe("runtime directories and helper boundary", () => {
  ...
  test("runs workbench commands without output limits or credentials", async () => {
    const root = await temporaryDirectory();
    ...
    await writeFile(
      ...
      [
        ...
        "assert.equal(process.argv[2], 'test-command');",
        "for (const key of ['OPENAI_API_KEY', 'CODEX_API_KEY', 'OPENROUTER_API_KEY', 'FIREWORKS_API_KEY']) assert.equal(process.env[key], undefined);",
        "const payload = readFileSync(0, 'utf8');",
        "console.log(JSON.stringify({ok: true, label: '出力', inputLength: payload.length, details: 'x'.repeat(5 * 1024 * 1024)}));",
      ].join("\n"),
    );
    const result = await runWorkbench(
      {
        ...
        environment: {
          ...
          CODEX_API_KEY: "also-must-not-reach-helper",
          OPENROUTER_API_KEY: "openrouter-must-not-reach-helper",
          FIREWORKS_API_KEY: "fireworks-must-not-reach-helper",
        },
      },
      ["test-command"],
      "x".repeat(64 * 1024),
  ...
  test.each([
    ...
    async (_kind, version, supportsStdin, supportsRelated, columns) => {
      ...
      const helpers = join(pluginRoot, "mcp");
      ...
      await writeFile(
        join(helpers, "helpers.mjs"),
        [
          "import {appendFileSync, readFileSync} from 'node:fs';",
          "const args = process.argv.slice(2);",
          "if (args.includes('--help')) {",
          "  appendFileSync(new URL('./help-calls', import.meta.url), 'help\\n');",
          "  if (process.env.FAIL_COMPARISON_HELP) { console.error('Synthetic help failure'); process.exit(1); }",
          `  console.log('save-scan-comparison --before-scan-id BEFORE --after-scan-id AFTER --matches-json MATCHES${supportsStdin ? " --matches-json-stdin" : ""}');`,
          ...(supportsRelated
            ? [
                "  console.log('Comparison payload supports\\nrelated findings.');",
              ]
            : []),
          "  process.exit(0);",
          "}",
          "const usesStdin = args.includes('--matches-json-stdin');",
          ...(supportsStdin
            ? []
            : [
                "if (usesStdin) throw new Error('Unsupported stdin transport');",
              ]),
          "const payload = JSON.parse(usesStdin ? readFileSync(0, 'utf8') : args[args.indexOf('--matches-json') + 1]);",
          ...(supportsRelated
            ? []
            : [
                "if ('related' in payload) throw new Error('Unsupported comparison fields');",
              ]),
          "console.log(JSON.stringify({payload, usesStdin}));",
        ].join("\n"),
      );
      const options = {
        pluginRoot,
      ...
      expect(await runWorkbench(options, args, input)).toEqual(expected);
      expect(await readFile(join(helpers, "help-calls"), "utf8")).toBe(
        "help\nhelp\n",
      );
  ...
  test("reports an unwritable SQLite state directory with actionable recovery", async () => {
    const root = await temporaryDirectory();
    const pluginRoot = join(root, "plugin");
    ...
    expect(message).not.toContain("helpers.mjs");
  });
